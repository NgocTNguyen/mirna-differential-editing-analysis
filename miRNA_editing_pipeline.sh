...
awk '$6!~/snp/ && $7!~/-/' total_combined_count_table | sort -u > internal_modification_count_table # select internal modifications only
...
#	for base in A U C G adar; do
		echo $indiv $base
		./modification_count.py $indiv $base # sum up all counts for each modification position of each miRNA
...
#for base in A U C G adar; do
	for method in kit uc; do
		for site in `cat $base.$method.sites_for_DE_testing`; do
			echo $site
			./identify_differentially_edited_site.R $site
		done
	done
done
...
#for base in A U C G adar; do
	for method in kit uc; do
		for group in C KA 24h 2w; do
			...
			./decide_differentially_edited_site.R $base $method $group
		done
	done
done
