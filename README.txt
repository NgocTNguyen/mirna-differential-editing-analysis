...
3. miRNA_editing_pipeline.sh is a master bash script to process the raw count data table (i.e. subtotal_combined_count_table) and call all relevant Python (sumup_modification_count.py) and R (identify_editing_sites.R, identify_differentially_edited_sites.R, decide_differentially_edited_sites.R) scripts to process the data and run all relevant statistical tests.
